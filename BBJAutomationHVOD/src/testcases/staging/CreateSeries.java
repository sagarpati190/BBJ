...
public class CreateSeries extends Utility {
  ...
  public void CreateSeries() {
	  ...
	  try {
		   ...
		   /* --------------------------Login Start ------------------------------------- */
	/*	  StagingScheduleEntires scheduleEntry1 = new StagingScheduleEntires(driver); 
		  String MAMFolder1 = "E2E_STEREO_SD_only";
		  String strAssetID1 = "BBJSampl12";
	   	  result = scheduleEntry1.uploadFile(strAssetID1,MAMFolder1);
	   	  
	   	  MAMFolder1 = "E2E_STEREO_ABR_HLS";
	   	  result = scheduleEntry1.uploadFile(strAssetID1,MAMFolder1);*/
		  	...
		  	/* --------------------------Catalog Start ------------------------------------- */  
		  	  ...
		  	  Assert.assertTrue(result, "Failed to Select Create Series"); 
		   	  reporter.logWithScreenshot("Create Series Button Clicked Successfully");
		    
		  	/* --------------------------Catalog End ------------------------------------- */ 
		  	  
		   	/* --------------------------Create Page Start ------------------------------------- */   
		   	  StagingCreateContent createSeries = new StagingCreateContent(driver);	  
		   	  
		   	  String strSeriesName=xlRead("AssetTitle");
		   	  result = createSeries.setSeriesName(strSeriesName);
		   	  Assert.assertTrue(result, "Failed to Enter Series Name"); 
		   	  reporter.log("Series Name Entered Successfully");
		   	  
		   	  String strLicensor=xlRead("Licensor");
		   	  result = createSeries.setLicensorSeries(strLicensor);
		   	  ...
		   	  if(! strBrandName.isEmpty()){
		   		  result = createSeries.setBrandName(strBrandName);
			   	  Assert.assertTrue(result, "Failed to Enter Brand  Name"); 
			   	  reporter.log("Series Brand Name Entered Successfully");
		   	  }
		   	  
		   	  int EpiNum = Integer.parseInt(xlRead("EpisodeNum"));
		      result = createSeries.addEpisodeSeries(EpiNum);
		   	  Assert.assertTrue(result, "Failed to click Add Episode button"); 
		   	  reporter.log("Add Episode button clicked Successfully");
		   	  
		   	  
		   	  result = createSeries.setEpisodeSeries(strSeriesName );
		   	  Assert.assertTrue(result, "Failed to click Add Episode button"); 
		   	  reporter.log("Add Episode button clicked Successfully");
		   	  
		   	 
		   	  result = createSeries.clickSubmit();
		   ...
		   /* --------------------------Create Page End ------------------------------------- */
		      ...
		      for(int j=1; j<= EpiNum;j++){
		    	  ...
		    	  Assert.assertTrue(result, "Failed to click Episodes Link"); 
			   	  reporter.logWithScreenshot("Episodes link Clicked Successfully");
				 ...
				 /*------------------------Title Page Start ------------------*/
			  	  ...
			  	  System.out.println("Title Name:"+strEpiName);
			  	  result = title.setTitleName(strEpiName);
...
//			  	 }
			...
			Assert.assertTrue(result, "Failed to Click Metadata "); 
		  	...
		  	Assert.assertTrue(result, "Failed to Set Collection Original Name"); 
	   	    ...
	   	    String strSessionNumber  = xlRead("SeasonNumber");
			...
			String strStandalone = xlRead("Standalone");
			
			// to edit and update vosp entires
		   	for(int i = 0 ;i < vospPlatform.size();i++){
				...
				String strContract = null;
			 	...
			 	result = editSchedule.setAsset(vospPlatform.get(i),EpiName);
			   	...
			   	if(i==0 && strStandalone.equals("Yes")){
			   		result = editSchedule.setStandalone(vospPlatform.get(i));
			   		Assert.assertTrue(result, "Failed to Set standalone"); 
				   	reporter.log("Stanalone Set succeessfully");
			   	}
		   	...
		   	// to edit and update Youview entires
		    ...
		    youviewPlatform =scheduleEntry.getYouviewPlatformId();
		   	...
		   	for(int i = 0 ;i < youviewPlatform.size();i++){
			 	...
			 	String EpiName  = scheduleEntry.getEpiName(youviewPlatform.get(i));
				...
				String strContract = null;
			   	...
			   	}else if(PlatformVarient.get(youviewPlatform.get(i)).equals("EST")){
			   		strContract = xlRead("ESTContract");
			   		result = editSchedule.setContract(strContract, youviewPlatform.get(i));
			   		Assert.assertTrue(result, "Failed to set EST Contract"); 
				   	reporter.log("EST Contract Set succeessfully");
				...
				if(i==0 && strStandalone.equals("Yes")){
			   		result = editSchedule.setStandalone(youviewPlatform.get(i));
			   		Assert.assertTrue(result, "Failed to Set standalone"); 
				   	reporter.log("Stanalone Set succeessfully");
			   	}
		   ...
		   /*	if(xlRead("STVOD").equalsIgnoreCase("Yes")){
		    ...
		    for(String key : editSchedule.AssetID.keySet()){
			    ...
			    Assert.assertTrue(result, "Failed to upload media file"); 
			   	reporter.log("Media File uploaded successfully");
		  	...
		  	for(String key : editSchedule.AssetID.keySet()){
			    ...
			    vospPlatform =scheduleEntry.getVospRenditions(strAssetID);
		  		...
		  		result = main.clickCollSerlink(strSeriesName);
